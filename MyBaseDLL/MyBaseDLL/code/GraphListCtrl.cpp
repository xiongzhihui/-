...
BOOL RegisterWndClass(HINSTANCE hInstance, LPCTSTR lpszClassName, UINT style, HICON hIcon = 0, HBRUSH hbrBackground = 0)
{
	WNDCLASS wndcls;
...
BOOL CGraphListCtrl::RegisterWindowClass()
{
	return RegisterWndClass(AfxGetInstanceHandle(), GRAPHCTRL_CLASSNAME, CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW);
}
...
{
	m_TitleTip.Show(rc, szTitle, 0, NULL, &m_font, GetTextColor(), GetBkColor());
}
...
// // 	HMODULE hModel = AGControlPackage_Arx.ModuleResourceInstance();
// 	HRSRC hRs = ::FindResource(NULL, MAKEINTRESOURCE(IDR_DWGDEFAULTICON), _T("DWG"));
// 	if (!hRs)
// 	{
...
// 		if (hModel)
// 		{
// 			hRs = ::FindResource(NULL, MAKEINTRESOURCE(IDR_DWGDEFAULTICON), _T("DWG"));
// 		}
// 	}
// 
// 	if (!hRs)
...
int CGraphListCtrl::GetItemCursel()
{
	return m_nCurSel;	
}
